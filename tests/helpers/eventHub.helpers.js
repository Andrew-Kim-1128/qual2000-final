...
export async function fillAdminEventForm(page, event) {
	...
	await page
		...
		.fill(String(event.availableSlots));
}
...
export async function createEventAsAdmin(page, overrides = {}) {
	...
	const addEventLink = page
		.getByRole("link", { name: /add( new)? event/i })
		.first();
	...
	await page
		.locator('form[action="/admin/events"]')
...
export async function ensureBaselinePublicEvents(page) {
	...
	const baselineEvents = [
		...
		},
		{
			...
			description:
				"Baseline public food event for registration coverage.",
			availableSlots: 200,
		},
	];
...
export async function registerForEvent(
	...
	await expect(page).toHaveURL(/\/events\/registrations/);
	await expect(
		page.getByText(/registration created successfully/i),
	).toBeVisible();
}
