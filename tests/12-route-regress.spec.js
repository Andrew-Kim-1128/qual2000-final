...
test.describe("Route Handline / Regression", () => {
    test("REGRESS-001 - create registration, edit seats, delete registration", async ({ page }) => {
        ...
        const row = await page.locator("tr").filter({ hasText: event.title }).first();
    ...
    test("REGRESS-002 - create event, edit event, delete event", async ({ page }) => {
        const event = await createEventAsAdmin(page);
