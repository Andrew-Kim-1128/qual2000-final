...
test.describe("Admin Edit", () => {
    ...
    test("ADMEDT-POS-001 - Edit Event page loads", async ({ page }) => {
        ...
        await expect(page.getByLabel(/available slots/i)).toHaveValue(String(event.availableSlots));
    });
    ...
    test("ADMEDT-POS-004 - update is visible on public site", async ({ page }) => {
        const newTitle = "new title";
...
test.describe("Invalid Admin Edit Event", () => {
    test("ADMEDT-NEG-001 - blank fields are rejected", async ({ page }) => {
    ...
    test("ADMEDT-NEG-002 - negative Available Slots are regected", async ({ page }) => {
        const event = await createEventAsAdmin(page);
    ...
    test("ADMEDT-NEG-005 - duplicate event rejected", async ({ page }) => {
        const events = [await createEventAsAdmin(page), await createEventAsAdmin(page)];
        ...
        await page.getByLabel(/available slots/i).fill(String(events[0].availableSlots));
