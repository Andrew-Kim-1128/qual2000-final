...
test.describe("Admin Delete Event", () => {
	...
	test("ADMDEL-POS-003 - deleted event is not visible on public site", async ({
		page,
	}) => {
		...
		await expect(
			page.getByText(/That page is not part of Event Hub/),
		).toBeDefined();
	});

	test("ADMDEL-POS-004 - deleted event is not visible on registrations", async ({
		page,
	}) => {
		await goto(page, "/events");
		...
		await page
			.getByRole("link", { name: "Register For This Event" })
			.click();

		await page
			.locator(`form[action="${url}/register"]`)
